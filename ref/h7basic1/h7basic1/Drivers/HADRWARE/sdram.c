...
uint8_t SDRAM_Test(void)
{
	...
	SPI_PrintWithPrefix("SDRAM", LCD_BLUE, "WriteTest Begin");
...
// 写入 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	...
	char inst[48];
  sprintf(inst, "result:%.2f MB/s", ExecutionSpeed);
  SPI_PrintWithPrefix("SDRAM", LCD_BLUE, inst);
  HAL_Delay(400);
// 读取	>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 
  SPI_PrintWithPrefix("SDRAM", LCD_BLUE, "ReadTest Begin");
	ExecutionTime_Begin 	= HAL_GetTick();	// 获取 systick 当前时间，单位ms
...
// 数据校验 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   
	...
	SPI_PrintWithPrefix("SDRAM", LCD_BLUE, "Validating Data");
	...
	for(i = 0; i < SDRAM_Size/2;i++ )
	{
		...
		{
			SPI_PrintWithPrefix("SDRAM", LCD_BLUE, "TEST FAILED");
			return ERROR;	 // 返回失败标志
		}
	}
	HAL_Delay(400);
	SPI_PrintWithPrefix("SDRAM", LCD_BLUE, "16Bit Test PASS");
  SPI_PrintWithPrefix("SDRAM", LCD_BLUE, "8Bit Test Begin");
  HAL_Delay(400);
	for (i = 0; i < 255; i++)
	{
  ...
  HAL_Delay(400);
	for (i = 0; i < 255; i++)
	{
		...
		{
			SPI_PrintWithPrefix("SDRAM", LCD_BLUE, "8Bit Test FAILED");
			SPI_PrintWithPrefix("SDRAM", LCD_BLUE, "Check NBL0/NBL1");
			return ERROR;	 // 返回失败标志
		}
	}		
	SPI_PrintWithPrefix("SDRAM", LCD_BLUE, "8Bit Test PASS");
